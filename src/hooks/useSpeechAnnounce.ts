...
import { speakNumber, stopSpeech } from '../utils/speech';
...
export function useSpeechAnnounce(settings: Settings) {
  ...
  const tickRaf = useCallback((timestamp: number) => {
    ...
    if (!cooldownRef.current && rawDb > effectiveThreshold) {
      ...
      // Speak the number
      const { voiceLang, selectedVoiceURI } = settingsRef.current;
      speakNumber(countRef.current, voiceLang, selectedVoiceURI);
